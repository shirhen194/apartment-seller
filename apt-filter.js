...
const findRelevantApts = function (address, minPrice, maxPrice, minRooms, maxRooms, immediate, parking) {
    ...
    let relevantAptsWithImmediate = immediate ? relevantApts : relevantApts.filter(a => (a.immediate))
    let relevantAptsWithImmediateAndParking = parking ? relevantApts : relevantApts.filter(a => (a.parking)) 

    return relevantAptsWithImmediateAndParking;
}
